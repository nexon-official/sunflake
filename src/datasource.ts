import { CoreApp, DataFrame, DataFrameView, DataSourceInstanceSettings, LegacyMetricFindQueryOptions, MetricFindValue, ScopedVars, TimeRange, VariableWithMultiSupport, getDefaultTimeRange } from '@grafana/data'
...
export class DataSource extends DataSourceWithBackend<SunflakeState, SunflakeDataSourceOptions> {
  constructor(instanceSettings: DataSourceInstanceSettings<SunflakeDataSourceOptions>) {
  ...
  async getSnowflakeColumns(database?: string, schema?: string, table?: string, searchWord?: string): Promise<TableColumn[]> {
